...
<ul>
   ...
   <li  class='active'><a href='deposit.php'><span>Deposit</span></a></li>
...
<table width="94%" border="0" cellspacing="0" style="background-color:#0861b5; margin:0px auto; width:950px; color:rgb(255,255,255)" align="center">
  <tr>
    ...
    <form action="deposit.php" method="post" enctype="multipart/form-data" name="admindes"
    onsubmit="return validation()">
  <tr>
    <th colspan="2" scope="col" align="center">
    <img src="images/deposit.jpeg" width="236" height="74" style="margin-left:30px;" /></th>
    </tr>
  <tr>
    <th scope="row"></th>
    <td align="left">&nbsp;</td>
  ...
  <tr>
    <th scope="row" align="right">Deposit  :</th>
    <td align="left"><label for="username2"></label>
      <input type="text" name="deposit" id="deposit" /></td>
  </tr>
  <tr>
...
function validation() {
	...
	var frm = document.admindes;
	   ...
	   if(frm.pinno.value =="") { alert("Please Enter the Pin No."); frm.pinno.focus(); return false }
	    if(frm.deposit.value =="") { alert("Please Enter the Deposit Amount."); frm.deposit.focus(); return false }
...
if(isset($_REQUEST['submit'])) { 
	...
	$sms = "Select * from deposit where username = '".$_SESSION['userid']."'";
	//echo $sms;
	//exitl
	$fgh=mysql_query($sms);
	$count = mysql_num_rows($fgh);
	$rowtrs=mysql_fetch_object($fgh);
	$depamts=$rowtrs->dep_amt;
	if($count == '0'){
	$sql = "insert into deposit(username,dep_accno,dep_pinno,dep_amt)
	values('".$_SESSION['userid']."','".$_POST['accno']."','".$_POST['pinno']."','".$_POST['deposit']."')";
		...
		$totamts = $rowtrs->dep_amt + $_POST['deposit'];
	   $sql = "Update deposit set dep_amt = '".$totamts."' where dep_accno = '".$_POST['accno']."' ";
		//echo $sql;
		//exit;
